...
import { getGroupDetail, joinGroupByCode } from "@/features/groups/services"
...
export const GroupDetailPage = ({ groupId }: GroupDetailPageProps) => {
  ...
  useEffect(() => {
    getGroupDetail(groupId)
      .then(setGroup)
      .catch((e) => toast.error(e.message ?? "Erro ao carregar grupo"))
      .finally(() => setIsLoading(false))
  }, [groupId])

  if (isLoading) return <PageContainer><DetailSkeleton /></PageContainer>
    ...
    <PageContainer>
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button asChild variant="outline">
          <Link href="/grupos"><ArrowLeft className="size-4" />Voltar aos grupos</Link>
        </Button>
      </div>
    </PageContainer>
  )
      ...
      <div className="flex flex-col gap-8">
        <FadeIn>
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <Button asChild variant="ghost" size="icon"><Link href="/grupos"><ArrowLeft className="size-4" /></Link></Button>
            ...
            <div className="flex flex-wrap items-center gap-3 pl-12">
              <Badge variant="secondary" className="gap-1.5">
                <Users className="size-3.5" />{group.memberCount} membros
              </Badge>
              <WeeklyTimer resetDate={group.weeklyReset} />
              <InviteLinkDialog inviteCode={group.inviteCode} groupName={group.name} />
            </div>
          </div>
        </FadeIn>
        <Separator />
        <FadeIn delay={0.1}>
          <div className="rounded-xl border border-border bg-card p-6">
            <h3 className="mb-2 text-center text-lg font-semibold text-foreground">Top 3 da Semana</h3>
            <RankingPodium members={group.members} />
          </div>
        </FadeIn>
        <FadeIn delay={0.2}><GroupStats group={group} /></FadeIn>
        <FadeIn delay={0.3}><RankingTable members={group.members} /></FadeIn>
        <FadeIn delay={0.4}><GroupMembersList members={group.members} createdBy={group.createdBy} /></FadeIn>
      </div>
    </PageContainer>
  )
}
